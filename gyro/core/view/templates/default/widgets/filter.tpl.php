...
if ($filter_group->count() > 1) {
	...
	foreach ($filter_group->get_filters() as $filter_item) {
		$arr_links[] = html::a(
			GyroString::escape($filter_item->get_title()),
			$filter_url_builder->get_filter_link($filter_item, $filter_group->get_group_id()),
			tr(
				'Filter by %name', 
				array('app', 'core'),  
				array('%name' => GyroGyroString::escape($filter_item->get_title()))
			)
		);
	}
	...
	$filter_html = tr(
		...
		array('app', 'core'), 
		array(
			'%name' => GyroGyroString::escape($filter_group->get_name()),
			'%current' => GyroGyroString::escape($filter_current->get_title()),
			'%values' => $values		
		)		
	);
	?>
